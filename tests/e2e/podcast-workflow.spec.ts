...
test("Editor completes the structured Podcast publication lifecycle", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "New artist" }).click(); await page.getByLabel("Artist name").fill(artistName); await page.getByRole("button", { name: "Create draft" }).click(); await page.getByRole("button", { name: "Publish now" }).click(); await expect(page.locator(".summary-strip")).toContainText("PUBLISHED");
  await page.getByRole("link", { name: "Podcasts", exact: true }).click(); await page.getByRole("link", { name: "Create Podcast" }).click(); await page.getByLabel("Title").fill(originalTitle); await page.getByLabel("Search Artists").fill(artistName); const podcastArtistValue = await page.getByLabel("Primary Artist").locator("option").filter({ hasText: artistName }).getAttribute("value"); await page.getByLabel("Primary Artist").selectOption(podcastArtistValue!); await page.getByLabel("Label").selectOption({ label: "Inner Symphony" }); await page.getByLabel("Episode Date").fill("2026-07-08"); await page.getByLabel("Duration").fill("01:03:45"); await page.getByRole("button", { name: "Create draft" }).click();
  await expect(page).toHaveURL(/\/admin\/podcasts\/[0-9a-f-]+$/); const legacyId = await page.locator(".summary-strip .mono").first().textContent(); await page.getByRole("button", { name: "Publish now" }).click(); await expect(page.locator(".alert.error")).toContainText("Artwork is required"); await page.getByLabel("Artwork").selectOption(artworkId); await page.getByRole("button", { name: "Add Chapter" }).click(); await page.getByRole("button", { name: "Add Chapter" }).click();
  ...
  await page.getByLabel("Chapter 1 Duration").fill("03:45"); await page.getByLabel("Chapter 2 Artist").fill("Closing Artist"); await page.getByLabel("Chapter 2 Title").fill("Closing Track"); await clickAndWaitForReload(page, "Save Draft"); await expect(page.getByText("Not visible to legacy clients until published.")).toBeVisible();
  let api = await page.request.get(`/index.php/cms/api/${legacyId}?filter=tracks&type=podcast`, { headers: { "X-Csrf-Token": apiKey } }); expect((await api.json()).tracks).toEqual([]); await page.getByRole("button", { name: "Publish now" }).click(); await expect(page.locator(".alert.error")).toContainText("Audio is required"); await page.getByLabel("Audio", { exact: true }).selectOption(audioId); await clickAndWaitForReload(page, "Save Draft");
  await clickAndWaitForReload(page, "Publish now"); await expect(page.getByTestId("legacy-preview")).toContainText(`Podcast E2E ${suffix}`); api = await page.request.get(`/index.php/cms/api/${legacyId}?filter=tracks&type=podcast`, { headers: { "X-Csrf-Token": apiKey } }); let delivered = (await api.json()).tracks[0]; expect(delivered.title).toBe(`Podcast E2E ${suffix}`); expect(delivered.artist_feature_times.map((chapter: { title: string }) => chapter.title)).toEqual(["Opening Track", "Closing Track"]);
  await page.getByLabel("Title", { exact: true }).fill(draftTitle); await page.getByLabel("Chapter 2 Title").fill("Closing Track Edited"); await page.getByRole("button", { name: "Move Chapter 2 up" }).click(); await clickAndWaitForReload(page, "Save Draft"); await expect(page.getByText("Unpublished changes")).toBeVisible(); await expect(page.getByTestId("canonical-preview")).toContainText("Closing Track Edited");
  ...
  await clickAndWaitForReload(page, "Publish now"); await expect(page.getByTestId("legacy-preview")).toContainText(draftTitle); api = await page.request.get(`/index.php/cms/api/${legacyId}?filter=tracks&type=podcast`, { headers: { "X-Csrf-Token": apiKey } }); delivered = (await api.json()).tracks[0]; expect(delivered.title).toBe(draftTitle); expect(delivered.artist_feature_times[0].title).toBe("Closing Track Edited");
...
test("Viewer can inspect Podcasts but cannot mutate them", async ({ page }) => {
  ...
  const denied = await page.request.patch(`/api/admin/podcasts/${page.url().split("/").at(-1)}`, { data: {} }); expect(denied.status()).toBe(403);
});
