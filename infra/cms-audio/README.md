...
Limits: 512 MiB and 30 minutes; PCM WAV or MP3. The worker probes actual input and output bytes, enforces 128000 bit/s and two channels, and verifies duration within 150 ms.
...
The template pins these fields to the retained isolated cms-worker-test branch. Database/branch/account identifiers in this test template are configuration, not credentials. Production targets require a separately reviewed rollout. The retained AWS image is the previously validated image; this local stabilization does not deploy a new image. Apply the explicit identity variables with the next authorized worker rollout.
