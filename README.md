...
Playwright acceptance records can be previewed and removed from the dedicated local CMS database with:

```sh
npm run dev:cleanup-e2e
npm run dev:cleanup-e2e -- --apply
```

The first command is always a dry run. The apply form deletes only records matching the exact timestamped names and filenames generated by `tests/e2e`, plus their revisions, audit rows, relations, unreferenced media metadata, and non-ADMIN headless-browser sessions. It refuses every database except `steyoyoke_cms_local` on a loopback host, protects test-marked records referenced by non-test catalogue content, never deletes users or accounts, and never touches stored media files.
