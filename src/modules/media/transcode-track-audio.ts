...
import { TRACK_AUDIO_MAX_BYTES, TRACK_AUDIO_MAX_SECONDS } from "./track-audio-contract";
...
export async function probeAudio(path: string) {
  ...
  if (!Number.isFinite(duration) || duration <= 0 || duration > TRACK_AUDIO_MAX_SECONDS) throw new Error("Audio duration is outside the supported range.");
...
export async function transcodeTrackAudio(source: string, destination: string) {
  const size = (await stat(source)).size;
  if (!size || size > TRACK_AUDIO_MAX_BYTES) throw new Error("Audio exceeds the upload size limit.");
  const input = await probeAudio(source);
  ...
  const output = await probeAudio(destination);
