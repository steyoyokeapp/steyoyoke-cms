...
export async function processImage(bytes: Buffer) {
  ...
  return { sourceFormat: metadata.format!, sourceWidth: metadata.autoOrient.width, sourceHeight: metadata.autoOrient.height, sourceChecksum: checksum(bytes), variants };
}
