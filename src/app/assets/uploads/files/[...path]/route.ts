...
export async function GET(_request: Request, context: RouteContext<"/assets/uploads/files/[...path]">) {
  ...
  try { return new Response(new Uint8Array(await mediaStorage.read(variant.storageKey)), { headers: { "Content-Type": variant.mimeType, "Content-Length": String(variant.byteSize), "Cache-Control": "public, max-age=31536000, immutable", "X-Content-Type-Options": "nosniff" } }); }
