...
export async function GET(request: Request, context: RouteContext<"/legacy-audio/[filename]">) {
  ...
  try {
    const bytes = await localStorage.read(asset.sourceStorageKey); const baseHeaders = responseHeaders({ mimeType: asset.mimeType, byteSize: asset.byteSize }); const range = request.headers.get("range");
