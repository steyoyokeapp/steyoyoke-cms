...
export default async function ArtistPage({ params }: PageProps<"/admin/artists/[id]">) {
  ...
  const [artist, media] = await Promise.all([getArtist(actor, id), listMediaAssets(actor)]);
  ...
  return (
    <>
      ...
      <ArtistEditor artist={data} role={actor.role} mediaAssets={media.map(({ id: mediaId, originalFilename, compatibilityFilename, width, height, status }) => ({ id: mediaId, originalFilename, compatibilityFilename, width, height, status }))} />
    </>
  );
}
