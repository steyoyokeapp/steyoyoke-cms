...
export default async function TrackPage({ params }: PageProps<"/admin/tracks/[id]">) {
  ...
  const data: TrackEditorData = {
    ...
    spotifyUrl: track.spotifyUrl, beatportUrl: track.beatportUrl, traxsourceUrl: track.traxsourceUrl, bandcampUrl: track.bandcampUrl, appleMusicUrl: track.appleMusicUrl, soundcloudUrl: track.soundcloudUrl, artworkAssetId: track.artworkAssetId, audioAssetId: track.audioAssetId,
  ...
  return <><Link className="back-link" href="/admin/tracks">← Tracks</Link><section className="page-heading"><div><div className="eyebrow">Track record</div><h1>{track.title}</h1><p className="muted">Working draft and immutable delivery snapshots.</p></div></section><TrackEditor track={data} role={actor.role} artists={options.artists.map(({ id, name, legacyId }) => ({ id, name, legacyId }))} labels={options.labels.map(({ id, name, active }) => ({ id, name, active }))} mediaAssets={media.map(({ id, originalFilename, compatibilityFilename, width, height, status }) => ({ id, originalFilename, compatibilityFilename: compatibilityFilename!, width: width!, height: height!, status }))} audioAssets={audio.map(({ id, originalFilename, legacyAudioId, durationMs, status }) => ({ id, originalFilename, legacyAudioId, durationMs, status }))} legacyPreview={legacyPreview} /></>;
}
