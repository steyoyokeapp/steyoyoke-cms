...
export async function POST(request: Request) {
  try { requireTrustedMutation(request); return Response.json(await reserveTrackAudio(await actorFromHeaders(request.headers), await request.json(), "podcast"), { status: 201 }); }
