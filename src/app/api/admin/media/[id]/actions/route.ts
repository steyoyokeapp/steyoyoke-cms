...
import { purgeEligibleMedia, retireMedia, retryImageProcessing } from "@/modules/media/service";
import { runMediaProcessingJobs } from "@/modules/media/image-worker";
import { logSafeError } from "@/lib/logger";

const schema = z.discriminatedUnion("action", [z.object({ action: z.literal("retire") }), z.object({ action: z.literal("purgeEligible") }), z.object({ action: z.literal("retryProcessing") })]);
