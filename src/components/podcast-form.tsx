...
import { PodcastChapters, editableChapters, chapterPayload } from "./podcast-chapters";
...
export function PodcastForm({ podcast, artists, labels, role = "EDITOR", mediaAssets = [], audioAssets = [] }: { podcast?: PodcastEditorData; artists: TrackOption[]; labels: TrackOption[]; role?: string; mediaAssets?: ArtworkOption[]; audioAssets?: AudioOption[] }) {
  ...
  const [chapters, setChapters] = useState(() => editableChapters(podcast?.chapters ?? []));
  ...
  async function save(e: FormEvent) {
    e.preventDefault(); setError(""); if (!primary) return setError("Choose a Primary Artist from the search results."); setPending(true);
    try { const result = await request(podcast ? `/api/admin/podcasts/${podcast.id}` : "/api/admin/podcasts", podcast ? "PATCH" : "POST", { title, primaryArtistId: primary, secondaryArtistId: secondary, labelId: label, episodeDate: date, audioAssetId: audioId, artworkAssetId: artworkId, chapters: chapterPayload(chapters), ...(podcast ? { expectedWorkingVersion: podcast.workingVersion } : {}) }); if (podcast) window.location.reload(); else { router.push(`/admin/podcasts/${result.id}`); router.refresh(); } }
  ...
  return <div className="catalogue-editor cms-form-design">
    ...
    <form className="panel editor-form" onSubmit={save} onChange={() => setDirty(true)}>
      <TrackMediaUpload modern audioProfile="podcast" kind="audio" initial={initialAudio} disabled={disabled || pending} onBusy={setAudioBusy} onReady={a => { setAudioId(a.id); setDirty(true); }} />
      ...
      <PodcastChapters value={chapters} onChange={v => { setChapters(v); setDirty(true); }} disabled={disabled || pending} />
      {error && <p className="alert error" role="alert">{error}</p>}
      {!disabled && <div className="track-save-row"><button className="button primary" disabled={busy}>{pending ? "Saving…" : podcast ? "Save changes" : "Create podcast"}</button></div>}
    </form>
