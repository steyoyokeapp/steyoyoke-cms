...
export type TrackEditorData = {
  ...
  bandcampUrl: string | null; appleMusicUrl: string | null; soundcloudUrl: string | null; status: string; workingVersion: number;
...
export function TrackEditor({ track, role, artists, labels, legacyPreview }: { track: TrackEditorData; role: string; artists: TrackOption[]; labels: TrackOption[]; legacyPreview: unknown }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await result(await fetch(`/api/admin/tracks/${track.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, primaryArtistId, secondaryArtistId, labelId, durationMs: parseDuration(duration), ...links, expectedWorkingVersion: track.workingVersion }) })); window.location.reload();
  ...
  return <div className="editor-grid">
    <div className="editor-column">
      ...
      <section className="panel media-placeholder"><div className="eyebrow">Media</div><h2>Artwork and audio</h2><p className="muted">Media assets arrive in a later vertical slice. No upload or placeholder URL is created here.</p></section>
